...
export const Exercices = () => {
  ...
  return (
    <DefaultLayout className="pokedex">
      <H1Styled>Exercices</H1Styled>
      {showExercice1 ? (
        <Exo1 onClose={() => setShowExercice1(false)} />
      ) : (
        <ButtonStyled onClick={() => setShowExercice1(true)}>
          Show exercice 1
        </ButtonStyled>
      )}
      {showExercice2 ? (
        <Exo2 onClose={() => setShowExercice2(false)} />
      ) : (
        <ButtonStyled onClick={() => setShowExercice2(true)}>
          Show exercice 2
        </ButtonStyled>
      )}
      {showExercice3 ? (
        <Exo3 onClose={() => setShowExercice3(false)} />
      ) : (
        <ButtonStyled onClick={() => setShowExercice3(true)}>
          Show exercice 3
        </ButtonStyled>
      )}
    </DefaultLayout>
